...
```{r}
mobility %>% 
  filter(!sub_region_2 %in% nuts_ds$origin) %>% 
  select(sub_region_2) %>% 
  table() # I need to change these names
nuts_ds$origin %>% table()
```
...
mobility %>% 
  ...
  rename(origin = sub_region_2) -> mobility
```
...
data_2019_test <- 
  data_2019_test %>% 
  mutate(
    origin = to_factor(origin)
  )

```

```{r}
str(data_2019_test$origin)
```
...
Salen todos NAs. Hay que ver por qué. 
Revisar data_2019 para ver qué variables de geolocalización hay y qué se puede
utilizar para utilizar los datos NUTS: origin sólo tiene "BCN" y "MAD" Y NO
COINCIDEN CON NUTS.