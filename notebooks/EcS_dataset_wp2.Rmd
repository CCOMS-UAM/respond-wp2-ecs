...
BASELINE_FILEPATH <- file.path(read_ecs_folder("DB"),"Ola_3/Cohorte_2019/rawdata_c2019w1.dta")
COVID_FILEPATH <- file.path(read_ecs_folder("DB"), "Subestudio_COVID/Edad_con_salud_Fichero_Completo.dta")
```
...
data_COVID <- read.dta13(COVID_FILEPATH, encoding = "UTF-8", fromEncoding = "CP1252")
...
```{r dates format}
fecha2019 <- lubridate::ymd(data_2019$q0006_date)
...
## ALCOHOL CONSUMPTION
"C:\Users\Cristina\UAM\Marta Miret Garcia - Documentacion Edad con Salud\Edad con salud - Ola 3\Outcomes\Cohorte 2019\Outcome datasets\"
...
We have to see which IDs are on COVID substudy. To do so we execute:
...
We look at which cases are in both wave 1 and wave 2:
```{r}
ID_ECS_subs <- data_COVID_test %>%
                semi_join(data_2019_test, by = "ID_ECS") %>% select(ID_ECS)
#FIXME: Devuelve un caso menos que el resto de las observaciones COVID (1853 
#en vez de 1854)
```

A vector is created with the birth dates of these cases:

```{r}
birthday_COVID <- data_2019_test %>% 
  filter(ID_ECS %in% ID_ECS_subs$ID_ECS) %>% 
  select(q1010_birth_date) %>% pull()
```

U
```{r}
data_COVID %>% filter(ID_ECS == 2079)
```
Queda revisar el caso que se ha eliminado que no esta en línea base. 
### Variables creation
```{r}
data_COVID_test <-
  data_COVID_test %>% 
