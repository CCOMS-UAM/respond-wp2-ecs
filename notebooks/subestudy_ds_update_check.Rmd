...
```{r}
RAW_DS_PATH    <- file.path(SUBSTUDY_FOLDER, "Edad_con_salud_Fichero_Completo.sav")
...
#Checking SAV dataset

```{r}
# Load the two datasets
updated_dataset <- read_sav(MOD_PATH)
spss_dataset  <- read_sav("C:/Users/Cristina/Documents/workspace/ECS_Substudy_Mod_CRP.sav")
```
...
```{r}
spss_attributes  <- spss_dataset   |> attributes()
updated_attributes <- updated_dataset |> attributes()
diff_attributes    <- compare(
  spss_attributes, updated_attributes,
  max_diffs   = Inf,
  x_arg       = "spss",
  y_arg       = "updated",
  list_as_map = TRUE
...
```{r}
spss_var_attrs  <- spss_dataset   |> map(attributes)
updated_var_attrs <- updated_dataset |> map(attributes)
diff_var_attrs    <- compare(
  spss_var_attrs, updated_var_attrs,
  max_diffs   = Inf,
  x_arg       = "spss",
  y_arg       = "updated",
  list_as_map = TRUE
...
diff_values <- var_names |> imap_dfr(
  ~{
    spss_var  <- spss_dataset   |> select(ID_ECS, old = !!sym(.x))
    updated_var <- updated_dataset |> select(ID_ECS, new = !!sym(.x))
    
    full_join(spss_var, updated_var, by = "ID_ECS")   |>
      filter((old != new) | (is.na(old) != is.na(new))) |>
      mutate(
        across(
          -ID_ECS,
