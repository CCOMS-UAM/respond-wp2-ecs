...
```{r}
prueba <- data_2019_test %>% left_join(
  data_COVID_test %>% select(ID_ECS, FECHAFIN),
  by = "ID_ECS"
)
```

```{r}
time_between_BL_covid <- prueba %>%  
  mutate(
    time_between_BL_covid = q0006_date %--% FECHAFIN, #año-mes-dia
    time_between_BL_covid = as.period(time_between_BL_covid)
    #,time_between_BL_covid = years(time_between_BL_covid)
  ) %>% select(time_between_BL_covid)
```
...
bl_time <- data_2019_test %>%  
  mutate(
    ...
    bl_time = as.period(bl_time),
    bl_time = years(bl_time)
  ) %>% select(bl_time)
```
...
covid_time <- data_COVID_test %>%  
  mutate(
    ...
    covid_time = as.period(covid_time),
    covid_time = years(covid_time)
  ) %>% select(covid_time)
```
...
birthday_COVID <- data_2019_test %>% 
  mutate(
    birthday_COVID = case_when(
      (ID_ECS %in% ID_NAs_sub) ~ q1010_year,
      (ID_ECS %in% id_ecs_both) ~ q1010_birth_date
    )) %>% 
