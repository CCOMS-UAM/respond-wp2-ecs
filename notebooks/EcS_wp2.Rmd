...
data_COVID_test %>% 
  ...
  ) %>% select(age_cat) %>% freq_table(age_cat)
```
...
data_COVID_test <- data_COVID_test %>% 
  mutate(
    across(
      .cols = c("age_cat"),
      .fns = ~if_else(TIPO_ENT == 2 | TIPO_ENT == MISSING_DESIGN),
              fct_na_value_to_level(age_cat, level = "-991")
    )
  )
```
...
data_COVID_test %>% 
  ...
  miss_var_summary() %>% 
  data.frame #variables hthat have 95% of missing with this filter are character variables that we don't use here
```
