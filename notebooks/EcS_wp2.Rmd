...
#TODO: 1429 as "others" (now is 1944?)
```
...
At this point, the things that have changed is that the **levels** have been
modified.

```{r}
levels(data_2019_test$q0410k_other_r)
```

```{r}
data_2019_test %>% 
  select(q0410g_total_r, 
         q0410i_ss_r, 
         q0410j_government_r,
         q0410k_other_r) %>% 
  remove_val_labels()
...
We check that NAs are well recoded (-992, -993 & NAs)
...
We need to create another variable from these taking into account the -993 and
-992 as missing so that they are not taken into account when calculating the
...
Right now the NAs are MISSING_DESIGN (proxy) and NON_RESPONSE. While the proxy
are always the same, the NON_RESPONSE are different for each variable.
...
data_2019_test <- 
  ...
  mutate(
    ...
    #                     
    #                     q0410g_total_rec == "valid" &
    #                     q0410i_ss_rec == "DK" ~ 0, 
    #                     
    #                     q0410g_total_rec == "valid" &
    #                     q0410i_ss_rec == "refuse" ~ 0,
    #                   
    #                     
    #                     q0410g_total_rec != "valid" & (
    #                     (q0410i_ss_rec == "DK" & q0410j_government_rec == "DK") |
    #                     (q0410i_ss_rec == "DK" & q0410j_other_rec == "DK") |
    #                     (q0410i_other_rec == "DK" & q0410j_government_rec == "DK")
    #                     ) ~ NON_RESPONSE,
    #                     
    #                     q0410g_total_rec != "valid" & (
    #                     (q0410i_ss_rec == "refuse" & q0410j_government_rec == "refuse") |
    #                     (q0410i_ss_rec == "refuse" & q0410j_other_rec == "refuse") |
    #                     (q0410i_other_rec == "refuse" & q0410j_government_rec == "refuse")
    #                     ) ~ REFUSED_ANSWER
    #                     
    #                     ),
    # 
    # government = case_when(q0410g_total_rec == "valid" &
    #                     q0410j_government_rec == "valid" ~ q0410j_government_NA,
    #                     
    #                     q0410g_total_rec == "valid" &
    #                     q0410j_government_rec == "DK" ~ 0, 
    #                     
    #                     q0410g_total_rec == "valid" &
    #                     q0410j_government_rec == "refuse" ~ 0,
    #                     
    #                     q0410g_total_rec != "valid" & (
    #                     (q0410i_ss_rec == "DK" & q0410j_government_rec == "DK") |
    #                     (q0410i_ss_rec == "DK" & q0410j_other_rec == "DK") |
    ...
    #                     q0410k_other_rec == "valid" ~ q0410j_other_NA,
    #                     
    #                     q0410g_total_rec == "valid" &
    #                     q0410k_other_rec == "DK" ~ 0, 
    #                     
    #                     q0410g_total_rec == "valid" &
    #                     q0410k_other_rec == "refuse" ~ 0,
    #                     
    #                     q0410g_total_rec != "valid" & (
    #                     (q0410i_ss_rec == "DK" & q0410j_government_rec == "DK") |
    #                     (q0410i_ss_rec == "DK" & q0410j_other_rec == "DK") |
 ...
 #FIXME!!!!!!!!!!!!!
      ...
      q0410g_tot_NA + q0410i_ss_NA + q0410j_gov_NA + q0410k_other_NA)
```
...
Finally we got 91 as not assessed (proxy respondants) and 996 missing (default).
...
#FIXME: solved! there's surely another easier way to do this.
```
...
```
edad_vars <- grep("edad", names(df), value = TRUE)

```{r}
vector_cadena_888 <- grep(c("999"), names(data_2019_test), value = TRUE)
vector_cadena_888 <- pull(vector_cadena_888)


data_2019_test %>% select(where(is.factor)) %>% map(., levels) %>% filter(. == "888")

variables <- c("q0410g_total", "q0410i_ss", "q0410j_government", "q0410k_other")

dfprueba <- data_2019_test %>%
  mutate(
    across(
...
#TODO: Establish that was not evaluated in Wave 2.
```
...
```{r}
#TODO: That was not evaluated in Wave 2.
...
data_COVID_test <- data_COVID_test %>% 
  mutate(
    living_alone = recode_factor(living_alone,
      "0" = "2",
      "1" = "1"
    ),
living_alone = factor(
...
data_COVID_test %>% 
   ...
   colnames()
#TODO: Is there a composite score?
```
...
#FIXME: Error: negative length vectors are not allowed
```
...
```{r}
rm(anxiety, brs, brsw2, covidcc, depression, economic, flourishing, g1, g2, g3, g4, livingalone, loneliness_ss_si, lte, materialdep, negposaffect, occ_unique, personality, qol, socialsupw2, tobacco, prueba, alcohol, suicidality, negposaf)
```
...
# Step 5. Selecting necessary variables

Firstly, we import the Edad con Salud varible list for RESPOND.
```{r}
