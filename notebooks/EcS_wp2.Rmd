...
data_2019_test <-
  ...
  mutate(
    ...
    coresidence = factor(coresidence,
                         levels = c("ES", "2", "3", MISSING_DESIGN,
                                                REFUSED_ANSWER, 
                                                NON_RESPONSE),
                          labels = c("yes", 
                                    "no",
...
## Province
It will be useful to join mobility dataset.
```{r}
...
```{r}
saveRDS(prueba, "./dat/prueba.rds")

prueba_rds <- readRDS("./dat/prueba.rds")
```
...
```{r}
test <- df |> filter(wave == 2)
```

```{r}
saveRDS(test, "./dat/test.rds")

test_rds <- readRDS("./dat/test.rds")
```
