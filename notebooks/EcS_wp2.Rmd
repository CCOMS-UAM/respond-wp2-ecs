...
```{r}
library(readr)
occ_unique <- read_delim("C:/Users/Cris/Downloads/HC-workerCat.txt", 
...
All variables have 91 NAs (proxy respondents). But the numbers of `888` and
`999` could be different.
...
<!-- \*In variable_list.xlsx there is a duplicate row with social participation that -->

<!-- says "size_sn", but it is a variable that does not exist in the database or in the  -->

<!-- 2019 cohort documentation. -->
...
#FIXME: 'Error in mutate(): Problem while computing `q6613_club_r = case_when(...)
#Caused by error in 'case_When()
#Using recode() is no solution because variables are double, not factors.
```
