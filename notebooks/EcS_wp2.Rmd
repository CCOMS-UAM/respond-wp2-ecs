...
1. Create the difference in years between
birthdate and 2019 interview date (in years). 
2. Create `age_continuous`: if we
have the complete information, use the diff. just created to stablish the age of
the participants; if this data is missing, use `q1011_age`. 
3. Create `age_cat`
...
1 = never married (and no cohabiting) 2 = currently married/law-partner 3 =
cohabiting 4 = separated/divorced 5 = widowed
...
-   had less than monthly contact with children, other inmediate family and
...
## 3.4. & 3.5. Age
As in wave 1, there are IDs that have a corrupted date of birth. 
We check which ones they are.
```{r}
data_COVID_test %>% 
  filter(ID_ECS %in% ID_NAs) %>% 
...
```{r}
prueba <- data_2019_test %>% left_join(data_COVID_test, by = "ID_ECS") %>%
  arrange(ID_ECS, wave)
```
