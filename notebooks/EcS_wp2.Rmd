...
On the other hand, we need to make sure that each unique ID is represented
two times in the dataset, regardless of whether they completed one or both
assessment waves. As we are harmonizing wave by wave, this has to be added on the
COVID substudy database. 
...
data_2019_test <- data_2019_test |> select(ID_ECS, wave, everything())  
```
...
data_COVID_test <- data_COVID_test |> select(ID_ECS, wave, everything()) 
```
...
data_2019_test <-
  ...
  mutate(
    coresidence = "ES",
    coresidence = factor(coresidence,
...
subcategories, which are the ones that could be interesting for us to use. 
We'll try if using it as a filter is useful.
...
data_2019_test <-
  ...
  mutate(
    profession = case_when(
      ...
      hc_worker == "yes" ~  4, #hc_worker = healthcare
      q1510_catoccu ==  5 ~ 5,  
      q1510_catoccu ==  10  ~  6 ,
...
```{r}
data_2019_test <- data_2019_test |> mutate(province = case_when(q0101a_psu == 9 ~ "Barcelona",
...
network.
Missing values in the face-to-face social interaction items (section q6210)
were recoded as 1 (“Never”) when the corresponding social network item 
...
social network.
Missing values in the technology-mediated social interaction items 
(section q6210v) were recoded as 1 “Never”) when the corresponding social 
network item was 2 (“No”). Then, all responses to these items, plus 
additional item q6221v were summed up.
...
interactions with member of their social network.
Sum of social_int_ff_pre and social_int_vr_pre. Ranges from 15 to 75.
...
Whether the participant declares their personal relationships have been
affected by the lockdown.
SOLO3 recoded as -1, 0, or 1, to indicate whether personal relationships are 
perceived to heve worsened, stayed unchanged, or improved, respectively.

```{r}
data_COVID_test$SOLO3
...
data_COVID_test <- data_COVID_test %>% 
  mutate(
    across(
      .cols = where(is.numeric),
      .fns = ~if_else(TIPO_ENT == 2 | is.na(TIPO_ENT), #Alternativa == 2,
                     replace_na(., MISSING_DESIGN),
                     .)
    ) 
  )
...
in wave 2. This will have to be changed later with the full database (they 
have to be "MISSING_DESIGN").
...
df <- df %>% mutate(
  across(
    .cols = c(RegionName, RegionCode, `C1M_School closing`:`V2A_Vaccine Availability (summary)`, `V2D_Medically/ clinically vulnerable (Non-elderly)`:`ConfirmedDeaths`, `PopulationVaccinated`:EconomicSupportIndex_ForDisplay),
    .fns = ~if_else(wave == 1,
                     replace_na(., MISSING_DESIGN),
                     .)
  )
...
We have to correct missing values. There are two types of missing values. 
For cases on wave == 1, they must be "MISSING_DESIGN" (as they not apply on 
baseline cases)
For cases on wave == 2, they must be "NON_RESPONSE" (as they apply in wCOVID,
but there is no response for them).

Also, we have to keep in mind that variables from mobility dataset have different
structure. Variables `country_region_code:sub_region_2` and `iso_3166_2_code` and
`place_id` are factors, while 
`retail_and_recreation_percent_change_from_baseline:residential_percent_change_from_baseline`
are integers. `metro_area` and `census_fips_code` are LOGI variables. We must apply
different strategies to treat NAs values. 
...
df <- df %>% 
  mutate(
    across(
      .cols = c(retail_and_recreation_percent_change_from_baseline:residential_percent_change_from_baseline, census_fips_code, metro_area),
      .fns = ~if_else(wave == 1,
                     replace_na(., MISSING_DESIGN),
                     .)
    ) 
...
df <- df %>% mutate(
  across(
    .cols = c(Intermediate_Region_Code, Intermediate_Region_Name, LDC:SIDS, `NUTS level 2`),
...
df <- df %>% group_by(ID_ECS) |> 
  mutate( 
    hashed_id = digest(ID_ECS, algo = "md5")) %>% 
...
Eliminamos las labels de los valores perdidos originales, que han sido sustituidos
por la familia de -991, -993
```{r}
#df %>% as_factor() %>% pull(q1510e_workday) %>% fct_drop(only = c("DK", "Refusal")) %>% levels()
...
#as_factor() %>% pull(q1510e_workday) %>% fct_drop(only = c("DK", "Refusal"))

```

# Stablishes "categories" for categorical variables in Opal
...
# Creates Annotations
```{r}
#TODO: PENDING!!!; not recognized in Opal
df %>% select(q1510e_workday)

attr(df$q1510e_workday, "label")

labelled::var_label(df$q1510e_workday)
...
label_codebook <- ecs_variablelist %>% 
  select(label, variable) %>%
  drop_na() %>%
  filter(variable %in% colnames(df))
...
test <- df %>% mutate(
  across(
    .cols = all_of(label_codebook$variable),
    .fns = ~sjlabelled::set_label(., labels_codebook$label)
  )
) #doesn't work (labels are the same)


test <- df %>% mutate(
  across(
    .cols = all_of(label_codebook$variable),
    .fns = ~sjlabelled::set_label(., label_codebook$label)
  )
) #Esto hace que todas las variables tengan como label toooodas las etiquetas

test <- df %>% mutate(
  across(
    .cols = all_of(label_codebook$variable),
    .fns = ~{labelled::var_label(.) <- labels_codebook}
  )
) #Doesn't work: "value should be a single character string or null

test <- df
labelled::var_label(test) <- labels_codebook 
#Error: `value` should be a single character string or NULL
...
```{r}
saveRDS(df, "./dat/df_test.rds")

test <- readRDS("./dat/df_test.rds")
```

```{r}
#Wave 1
w2019 <- df |> filter(wave == 1)
saveRDS(w2019, "./dat/c2019_test.rds")

wCOVID <- df |> filter(wave == 2)
saveRDS(wCOVID, "./dat/wCOVID_test.rds")
```

```{r}
freq_table(w2019, q1510e_workday)
```
